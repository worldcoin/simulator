...
async function checkFilesInCache(files: string[]) {
  ...
  return Promise.all(
    files.map(async (file) => {
      const fileInCache = await Promise.all(cacheKeys.map((key) => caches.open(key).then((cache) => cache.match(file))))
...
export const SEQUENCER_ENDPOINT: Record<Chain, Record<CredentialType, string | undefined>> = {
