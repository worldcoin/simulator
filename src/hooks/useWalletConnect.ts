...
import type {Chain, Identity, MetadataParams, SessionEvent, SignRequest, SignResponse, Verification} from '@/types'
...
function getHighestCredentialType(credentialTypes: CredentialType[]): CredentialType {
  // Orb credential always takes precedence over all other credential types
  return credentialTypes.includes(CredentialType.Orb) ? CredentialType.Orb : CredentialType.Phone // TODO: Add support for arbitrary number of credential types
}

function buildResponse(id: number, chain: Chain, credential_type: CredentialType, fullProof: FullProof): SignResponse {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- FIXME
  const proof = fullProof.proof as [bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint]
  ...
  return {
    ...
    result: {
      // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- FIXME
      merkle_root: encodePacked(['uint256'], [fullProof.merkleTreeRoot as bigint]),
      // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- FIXME
      nullifier_hash: encodePacked(['uint256'], [fullProof.nullifierHash as bigint]),
...
async function rejectRequest(id: number, topic: string, code: number, message: string): Promise<void> {
...
export const useWalletConnect = (ready?: boolean) => {
  ...
  const {setOpen, setStatus, metadata, setMetadata, setEvent, verification, setVerification, reset} =
    useModalStore(getStore)

  const identityRef = useRef(identity)
  const metadataRef = useRef(metadata)

  async function approveRequest(event: SessionEvent, credentialTypes: CredentialType[]): Promise<void> {
    ...
    const response = buildResponse(id, identityRef.current.chain, credentialType, verification.fullProof)
  ...
  const onSessionRequest = useCallback(
    async (event: SignClientTypes.EventArguments['session_request']): Promise<void> => {
      // Destructure session request
      const {
        id,
        topic,
      ...
      try {
        ...
        const _verification = await generateProof(identityRef.current, request)
        ...
        const _metadata = await fetchMetadata(params)

        if (!_metadata.is_staging) {
  ...
  const onSessionDisconnect = useCallback(
    async (event: SignClientTypes.EventArguments['session_delete']): Promise<void> => {
