...
export function V4PersonaEditor({ identity, onSave }: Props) {
  ...
  const save = () => {
    const age = form.age.trim() === "" ? Number.NaN : Number(form.age);
    const persona = {
      ...form,
      age,
    };
    const validationError = validateV4Persona(persona);
    if (validationError) {
      toast.error(validationError);
      return;
    }
    const normalizedPersona = normalizeV4Persona(persona);
    ...
    toast.success("Saved identity details");
  };

  return (
    <div className="mt-3 rounded-16 bg-gray-50 p-4">
      <div className="mb-4">
        <h3 className="text-s3">Identity Check details</h3>
