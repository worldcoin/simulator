...
export default function Maintenance() {
  ...
  const checkStatus = async () => {
    try {
      ...
      const sdkStagingNetwork = services.find((service) => service.name === 'SDK Staging Network')
  ...
  return (
    ...
    >
      <Icon name="warning" className="h-5 w-5 text-gray-0" bgClassName="bg-warning-700 h-8 w-8 rounded-full shrink-0" />
      <div className="ml-4 flex flex-col">
        <h3 className="font-rubik text-s2">Under maintenance</h3>
        <p className="mt-1 text-b3">
          Some features of the App are under maintenance. Functionality will be back to normal soon.
        </p>
      </div>
    </a>
  )
