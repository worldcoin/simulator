...
export default function VerifyItem(props: VerifyItemProps) {
  return (
    ...
    >
      {props.verified ? (
        <IconGradient name={props.icon} color={props.color} className="h-5 w-5" />
      ) : (
        <Icon name={props.icon} className="h-5 w-5 text-gray-400" bgClassName="h-10 w-10 bg-gray-200 rounded-12" />
      )}
    </Item>
  )
}
