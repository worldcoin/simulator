...
export const VerifyPhone = memo(function VerifyPhone(props: {
  ...
  return (
    <Drawer fullHeight open={props.open} onClose={props.onClose}>
      <div className="flex h-full flex-col">
        <Header iconLeft="direction-down" onClickLeft={props.onClose} />
        <IconGradient
          name="phone"
          color="#00C313"
        ...
        <h1 className="mt-8 text-center font-sora text-h1">Phone number verification</h1>
        <p className="mt-4 text-center text-b2 text-gray-500">
          Verifying your phone number is the equivalent of adding your number in the World app.
        </p>

        <Input value="+1 555 867 5309" className="mt-8" disabled />
        <p className="mx-2 mt-1 text-left text-b3 text-gray-400">Phone number is randomly simulated and unverified</p>

        <div className="grow" />
        <VerifyStatus status={status} handleClick={handleClick} />
      </div>
    </Drawer>
  )
})
