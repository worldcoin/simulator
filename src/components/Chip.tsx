...
export default function Chip() {
  ...
  return (
    <div className="flex h-8 items-center gap-1 rounded-full bg-info-100 px-3 text-s4 font-medium text-info-700">
      {isReady ? <Icon name="user" className="h-4 w-4" /> : <Icon name="spinner" className="h-4 w-4 animate-spin" />}

      {isReady && <span className="leading-[1px]">{identity?.persisted ? 'Persistent ID' : 'Temporary ID'}</span>}
