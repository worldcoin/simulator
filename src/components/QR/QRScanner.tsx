...
export const QRScanner = React.memo(function QRScanner(props: QRScannerProps) {
  ...
  return (
    <Dialog open={props.open} onClose={props.onClose} closeIcon="close">
      <h2 className="relative z-10 py-1.5 text-center text-h3 font-bold text-white">Scanner</h2>

      <div ref={containerRef} className={clsx('absolute inset-0 bg-black', props.className)}>
        {allowed !== false && (
          <Fragment>
            <video
              muted
            ...
            <QRFrame valid={valid} videoRef={videoRef} containerRef={containerRef} />
        ...
        {allowed === false && (
          <div className="absolute inset-x-0 top-1/2 grid -translate-y-1/2 items-center justify-items-center space-y-8">
            <span className="rounded-full bg-gray-100 p-7">
              <Icon name="camera-off" className="h-8 w-8 text-gray-400" />
            </span>

            <div className="space-y-4 px-14 text-center font-rubik text-white">
              <p className="text-20 font-semibold">Allow Worldcoin to access your camera</p>
        ...
        <div className="absolute inset-x-0 bottom-12 flex justify-center">
          <button className="flex flex-col items-center" onClick={props.onClickManualInput}>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-200">
              <Icon name="text" className="h-6 w-6" />
            </div>
