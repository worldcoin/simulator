    ...
    <Root>
        ...
        <div className="flex w-full items-center justify-between">
          {selected ? (
            <div className="flex items-center">
              <img src={selected.src} alt={selected.alt} className="h-8 w-8" />
              <span className="ml-3">{selected.label}</span>
            </div>
          ) : (
            <span>Select Chain</span>
          )}
          <img src="/icons/direction-down.svg" alt="direction-down" className="h-8 w-6" />
        </div>
      </Trigger>
      <Content className="z-10 flex w-full flex-col gap-y-2 rounded-12 border bg-white p-4 text-16">
        {filteredOptions.map((option) => (
          ...
          >
            <img src={option.src} alt={option.alt} className="h-8 w-8" />
