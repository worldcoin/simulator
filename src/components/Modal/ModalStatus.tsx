  ...
  return (
    <div className="my-8 flex items-center justify-center">
      {props.status === Status.Waiting && (
        ...
        >
          <>
            <Icon name="world-id" className="h-6 w-6" />
      ...
      {props.status === Status.Warning && (
        <>
          <Icon name="close" className="h-4 w-4 text-white " bgClassName="rounded-full w-6 h-6 bg-warning-700" />
          <span className="ml-2 text-16 font-semibold text-warning-700">You&apos;ve done this before</span>
        </>
      )}
