...
export default function ModalConfirm(props: ModalConfirmProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <Icon name="info" className="h-10 w-10 text-gray-900" bgClassName="h-20 w-20 rounded-full bg-gray-200 mt-8" />
      <h2 className="mt-8 text-center font-sora text-h1">
        {props.isVerified ? 'Identity pending' : 'Unverified identity'}
      </h2>
      <p className="mt-4 text-center text-b1 text-gray-500">{message.map((part) => part)}</p>
      <Button
