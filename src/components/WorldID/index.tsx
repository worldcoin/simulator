...
export function WorldID(props: {
  ...
  return (
    <div className={clsx(props.className, styles.card, 'h-full font-rubik xs:aspect-[327/435] xs:h-fit')}>
      <div
      ...
      >
        ...
        >
          ...
          >
            {props.verified && (
              <Icon name="bead-verified" className="h-[26.6vw] w-[26.6vw] xs:h-[100px] xs:w-[100px]" noMask />
            )}

            {!props.verified && (
              <Icon name="bead-not-verified" className="h-[26.6vw] w-[26.6vw] xs:h-[100px] xs:w-[100px]" noMask />
            )}
            <div className="space-y-0.5 text-center">
              <p
                className={clsx('text-[6.4vw] font-medium uppercase xs:text-24', {
          ...
          >
            ...
            <Icon
              name="logo"
              className={clsx('h-[6.4vw] w-[6.4vw] scale-x-[-1] xs:h-6 xs:w-6 ', {
                'text-gray-900': !props.verified,
                'text-white': props.verified,
              })}
            />
        ...
        >
          ...
          {props.verified && (
            <Fragment>
              ...
              >
                {`${'ORB<VERIFIED'}${dayjs(props.signAt).format('DDMMYYYY')}${'<<<<<'}${props.address}
                  `
                  .split('')
                  .map((i, key) => (
                    <span key={key}>{i}</span>
              ...
              <div className="absolute bottom-2 left-8 grid text-5 uppercase">
                <span className="text-white">Verified in {props.verifiedIn}</span>
                <span className="text-white">Expiration {dayjs(props.expiration).format('MMM DD, YYYY')}</span>
                <span className="text-white/20">World ID {props.widVersion}</span>
              </div>
            </Fragment>
          )}
        </div>
