...
const POLYGON_SEQUENCER_STAGING_PASSWORD: Record<CredentialType, string | undefined> = {
...
const OPTIMISM_SEQUENCER_STAGING_PASSWORD: Record<CredentialType, string | undefined> = {
...
const SEQUENCER_PASSWORD: Record<Chain, Record<CredentialType, string | undefined>> = {
  [Chain.Polygon]: POLYGON_SEQUENCER_STAGING_PASSWORD,
  [Chain.Optimism]: OPTIMISM_SEQUENCER_STAGING_PASSWORD,
}

function buildUrl(endpoint: string, chain: Chain, credentialType: CredentialType) {
  return new URL(endpoint, SEQUENCER_ENDPOINT[chain][credentialType])
}

function buildHeaders(authenticate: boolean, chain: Chain, credentialType: CredentialType): HeadersInit {
  const headers: HeadersInit = {
    'Content-Type': 'application/json',
  }

  if (authenticate) {
    headers.Authorization = `Basic ${btoa(`worldcoin:${SEQUENCER_PASSWORD[chain][credentialType]}`)}`
  }
...
async function postRequest<T = unknown>(request: SequencerRequest): Promise<T> {
  if (request.authenticate && !SEQUENCER_PASSWORD[request.chain][request.credentialType]) {
    throw new Error(
  ...
  const response = await fetch(buildUrl(request.endpoint, request.chain, request.credentialType).toString(), {
    method: 'POST',
    body: JSON.stringify({identityCommitment: request.commitment}),
    headers: buildHeaders(request.authenticate ?? false, request.chain, request.credentialType),
  })
...
export async function inclusionProof(chain: Chain, credentialType: CredentialType, commitment: string) {
...
export async function insertIdentity(chain: Chain, credentialType: CredentialType, commitment: string) {
